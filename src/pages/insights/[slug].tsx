...
export default function PostPage({ post }: PostPageProps) {
  ...
  return (
    <>
      <Head>
        <title>{post.title} - Involv</title>
        <meta name="description" content={post.excerpt || `Read ${post.title} on Involv`} />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
        
        {/* Open Graph tags */}
        <meta property="og:title" content={post.title} />
        <meta property="og:description" content={post.excerpt || `Read ${post.title} on Involv`} />
        ...
        <meta name="twitter:description" content={post.excerpt || `Read ${post.title} on Involv`} />
        {post.mainImage?.asset?.url && (
...
export const getStaticPaths: GetStaticPaths = async () => {
  // Get all posts that include 'involv' in their sites array
  const posts = await getPosts('involv', 100) // Get more posts for paths
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  try {
    ...
    // Check if this post is available for the 'involv' site
    if (!post.sites.includes('involv')) {
      return { notFound: true }
    }
